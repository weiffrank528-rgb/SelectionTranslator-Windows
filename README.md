...
> **Alpha 预览版（0.4.4）**：核心功能可用，但不同应用的可访问性实现差异很大。欢迎提交不含敏感内容的兼容性报告。

一个可运行的 Windows 桌面 MVP：在其他应用里用左键正常拖选文字，松开后自动读取选区，并在选区或鼠标附近显示简体中文翻译。无需按快捷键，也无需再点击“翻译”。

![SelectionTranslator 翻译浮窗](docs/images/popup-0.4.4.png)

## 当前能力

- 全局检测左键拖选与松开；钩子始终把原始鼠标消息继续传给目标应用，不吞点击、不改选区。
...
- 放大后的交互式卡片浮窗提供“复制原文”和“朗读原文/停止朗读”按钮。浮窗使用 `WS_EX_NOACTIVATE`，显示或点击按钮都不抢目标应用的键盘焦点，因此原应用中的 `Ctrl+C` 仍可正常使用。
- 翻译引擎可选：
  - MyMemory：默认、免 Key 的公共基础方案。
...
- 如果用户在鼠标松开后立即手动按下 `Ctrl+C`，程序会识别剪贴板序列变化并直接使用该文本，不再用自动取词快照覆盖用户刚复制的内容。
...
双击运行后应用常驻系统托盘。到浏览器、Word、PDF 阅读器、VS Code 或聊天软件里左键拖选一段英文并松开即可。右键托盘图标可暂停、打开设置或退出。

默认使用 MyMemory，源语言为 `en`，目标语言为 `zh-CN`。如果主要翻译其他语言，请在设置中修改源语言；Google/DeepL 可将源语言设为 `auto`。
...
兜底过程会在约 260 毫秒内短暂改动剪贴板，然后恢复原纯文本。0.4.1 起不再尝试克隆任意 Win32 剪贴板句柄：只有原剪贴板为空，或只包含 `CF_TEXT`、`CF_OEMTEXT`、`CF_UNICODETEXT`、`CF_LOCALE` 这些安全文本格式时才会执行 Ctrl+C。遇到图片、HTML、OLE 对象或其他注册格式时，本次兜底直接跳过，从根源上避免错误复制句柄导致的原生堆损坏。对剪贴板极敏感时，仍可在设置中关闭兜底，仅使用 UI Automation。
